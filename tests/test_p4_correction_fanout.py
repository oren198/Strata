...
who the correction must reach once g_root's own refresh reacts to it. The refresh
judge here OMITS withdraw_published (Aron's live finding, qwen's optional-field
pattern) — the ENGINE mechanically withdraws it instead (CEO decision A), since the
published item's content is verbatim-identical to the corrected claim.
...
def test_cross_scope_correction_fans_out_to_a_publication_reader_under_one_change_id(
    ...
    # --- step 2: g_root's OWN refresh judge reacts to the correction — but, per
    # Aron's live finding (qwen's optional-field pattern), OMITS withdraw_published
    # entirely. The ENGINE must catch it anyway (CEO decision A): the published
    # item's content is verbatim-identical to the corrected claim. -----------------
    def fake_judge(**kwargs: Any) -> ScopeManagerJudgment:
        ...
        return ScopeManagerJudgment(
            decision="accept_as_context",
            reasoning="The service's port claim was corrected.",
            new_summary=ScopeSummary(
                scope_id="g_root",
                directives=[],
    ...
    with RecordStore(db_path) as store:
        ...
        # g_reader is now told for the FIRST time — exactly once — through the
        # ENGINE's own withdrawal (the judge omitted withdraw_published), under the
        # SAME change id the cross-scope notice minted.
        ...
        # The mechanical withdrawal actually happened, with no judgment row of its
        # own (acceptance criterion 4 — never impersonating a judge act).
        assert store.list_publication_acts(scope_id="g_root")[-1].withdraws == act.id
        assert (
            store.get_publication_judgment(store.list_publication_acts(scope_id="g_root")[-1].id)
            is None
        )
...
def test_same_scope_correction_engine_withdraws_when_judge_omits_it(
    tmp_path: Path,
) -> None:
    """The same live finding, same-scope: the P3 judgment itself (not a refresh)
    is failed_corrected but omits withdraw_published. The engine still withdraws
    g_root's own published item and fans out to g_reader, under the P3 audit row's
    own change id."""
    from strata import app
    from strata.fleet_config import FleetConfig
    ...
    with RecordStore(db_path) as store:
        # g_root's own readers get the self-notice (born processed) plus g_reader
        # gets an ordinary fan-out — from the ENGINE's own withdrawal, not the judge's.
        reader_events = store.list_change_events(scope_id="g_reader")
        assert len(reader_events) == 1
        assert reader_events[0].kind == "claim_corrected"
        assert "port is unknown" in reader_events[0].after
...
def test_input_changes_block_instructs_the_refresh_judge_only_for_claim_corrected() -> None:
    """ADR 0017 P4 (philosopher's follow-up 1): new instruction text appears ONLY
    for a claim_corrected event — every other refresh kind's rendering stays
    byte-identical (the input-identity rule)."""
    from strata.record_store import ChangeEvent
    from strata.scope_manager import _render_input_changes

    def _event(kind: str) -> ChangeEvent:
        return ChangeEvent(
            id="ce_1",
            change_id="chg_1",
            contribution_id="c_notice",
            scope_id="g_reader",
    ...
    claim_corrected_block = _render_input_changes([_event("claim_corrected")])
    assert "IN OTHER WORDS" in claim_corrected_block
    assert "withdraw_published" in claim_corrected_block

    for kind in ("withdrawn", "published", "amended", "directive_appended"):
        block = _render_input_changes([_event(kind)])
        assert "IN OTHER WORDS" not in block
