...
def read_default_harness(project_root: Path) -> str | None:
    ...
    :func:`strata.install.read_default_harness`. Returns ``None`` when the
    project is not registered (no ``.strata/config.toml``), the file cannot
    be read, or no ``default_harness`` was ever set — every case a caller
    (``strata launch``'s harness resolution, Task 5) treats the same way:
    ...
    Args:
        project_root: The registered project's root directory (the directory
            containing ``.strata/``).
    """
    from strata.install import read_default_harness as _read_default_harness_toml
