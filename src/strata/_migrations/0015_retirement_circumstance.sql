-- Strata migration: a retirement records its CHANGED CIRCUMSTANCE (#209).
--
-- A scope-manager `retire` removes a directive with nothing replacing it, so the
-- record must say WHAT CHANGED: `changed_circumstance` is the circumstance that changed, in the
...
-- need none.
ALTER TABLE retirements ADD COLUMN changed_circumstance TEXT;
